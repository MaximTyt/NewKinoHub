...
namespace KinoHab.Manager
{
    public interface IFilmManager
    {
        ...
        Task<ICollection<Media>> GetAllFilms(int type);
        Task<Media> GetFilmforId(int filmId,Users User);
        MediaType TypeFilm(string i);
        Task<ICollection<Media>> AllSorting(string sort, Users User, int type);
        Task<ICollection<Media>> Filtration(int filtr, Users User, int type);
        ...
        Task AddFilm(IFormFile mainPhoto, string Name, string Contry, int Age, string RunTime, string Description, string shortDiscription, string ScoreKP, string Music, string Video, int NumOfEpisodes, int NumOfSeason, int type, IFormFile Images1, IFormFile Images2, IFormFile Images3, IFormFile Images4, string[] genres, DateTime Release_Date);
        Task EditFilm(IFormFile mainPhoto, string Name, string Contry, int Age, string RunTime, string Description, string shortDiscription, string ScoreKP, string Music, string Video, int Id, int NumOfEpisodes, int NumOfSeason, int type, IFormFile Images1, IFormFile Images2, IFormFile Images3, IFormFile Images4, string[] genres, DateTime Release_Date);
